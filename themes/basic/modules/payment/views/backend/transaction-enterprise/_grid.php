...
echo GridView::widget([
    ...
    'columns' => [
        ...
        [
            ...
            'value' => function ($model) {
                if (empty($model->tutor)) {
                    return '';
                }
                /**
                 * @var $model Transaction
                 */
                if ($model->isClientBalance()) {
                    return null;
                }
        ...
        [
            ...
            'template' => '{approve} {reject} {view} {refund} {recharge}',
            'visibleButtons' => [
                'refund' => function ($model) {
                    /**
                     * @var Transaction $model
                     */
                    return ($model->type == Transaction::STRIPE_CHARGE || ($model->isLessonTransfer()))
                ...
                'recharge' => function ($model) {
                    return $model->isAllowedReCharge();
                },
            ],
            'buttons' => [
                ...
                'recharge' => function ($url, $model, $key) use ($dataProvider) {
                    return Html::a('<i class="fa fa-reply" aria-hidden="true"></i>', $url, [
                        'title' => Yii::t('yii', 'recharge'),
                        'aria-label' => Yii::t('yii', 'recharge'),
                        'data-confirm' => Yii::t('yii', 'Are you sure you want to recharge this transaction?'),
                        'data-method' => 'post',
                        'data-pjax' => '0',
                    ]);
                },
