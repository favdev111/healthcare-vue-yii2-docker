...
define([
    ...
    Helpers.sendMessageProhibited = function(data) {
        window.parent.CommonHelper.showAllErrors(data);
    };

    Helpers.replaceForbiddenSymbols = function (string) {
      var symbols = App.chat.symbolsForbiddenInPhoneNumber;
      var count = 0;
      var replace  = false;
      for (var index in symbols) {
          var instring = (string.split(symbols[index]).length - 1);
          count+=instring;
          if (count >= App.chat.allowableCountForbiddenSymbols) {
              replace = true;
              break;
          }
      }
      ...
      if (replace) {
          for ( var index2 in symbols) {
              string = string.replace(new RegExp(symbols[index2], 'g'), App.chat.forbiddenSymbolReplacement);
          }
      }
