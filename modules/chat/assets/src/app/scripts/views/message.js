    ...
    MessageView.prototype = {
        ...
        addItem: function(message, isCallback, isMessageListener, recipientId) {
            ...
            if ((typeof recipientId !==  'undefined') && App.chat.user.chatId === recipientId) {
                if (
                    window.unverifiedStudents.indexOf(message.sender_id) !== -1
                    || window.unverifiedStudents.indexOf(message.recipient_id) !== -1
                ) {
            ...
            this.checkSenderId(message.sender_id, function() {
                ...
                switch (type) {
                    ...
                    default:
                        ...
                        html += '<div class="message-avatar avatar profileUserAvatar' + (message.stack ? ' is-hidden' : '') + ((message.sender_id != User.contact.id) && !window.App.identity.isTutor? ' click-avatar' :'')+
                            '" style="background-image:url(' + avatar_url + ')" data-id="' + message.sender_id + '"></div>';
                        html += '<div class="message-container-wrap">';
                        html += '<div class="message-container l-flexbox l-flexbox_flexbetween l-flexbox_alignstretch">';
                        html += '<div class="message-content' + (message.stack ? ' indent' : '') + '">';
                        html += '<h4 class="message-author' + (message.stack ? ' is-hidden' : '') + '"><span class="profileUserName" data-id="' + message.sender_id + '">' + full_name + '</span></h4>';
                ...
                if (window.App.identity.isTutor === true && checkStudent) {
                    var info = contacts[checkStudent];
                    if (!info.is_verified) {
                        var unverified = '<span class="user-check-chat user-check-chat--unverified" data-title="This student does not have a payment method on file & has been notified. For your own safety, please request that the student add a payment method on file before confirming any lessons.">' +
                            '<img src="images/icon-unverified.svg" alt="user unverified">' +
                            'Unverified</span>';
                    } else {
                        var unverified = '<span class="user-check-chat" data-title="You’re good to go! This student has a verified payment method on file and can be billed through your dashboard.">' +
                            '<img src="images/icon-verified.svg" alt="user verified">' +
                            'Verified</span>';
                    }
                    $('.message[data-id="' + checkStudent + '"] h4 .user-check-chat').remove();
                    $('.message[data-id="' + checkStudent + '"] h4').append(unverified);
                }
        ...
        sendMessage: function(form) {
            ...
            if (val.length > 0) {
                ...
                var getParamsRaw = decodeURIComponent(window.location.search.substr(1)).split('&');
                var getParams = [];
        ...
        onMessage: function(id, message) {
            ...
            if (
                window.unverifiedStudents.indexOf(msg.sender_id) !== -1
                || window.unverifiedStudents.indexOf(App.chat.user.chatId) !== -1
            ) {
