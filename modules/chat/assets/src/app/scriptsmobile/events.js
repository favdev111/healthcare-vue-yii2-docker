    ...
    Events.prototype = {
        ...
        init: function() {
            ...
            $(document).on('click', '.dialog-item', function() {
                $('.l-workspace-wrap').addClass('l-workspace-wrap-visible');
                $('.btn-link--back-page', top.document).removeClass('hidden');
                $('.dropdown--tutor-chat', top.document).removeClass('hidden');
                $('.l-list-wrap section .is-selected .last-message-dialog').removeClass('unread-dialog-message');
            });

            $('body').on('click touchstart', '#add-student-from-chat-link', function (e) {
                e.preventDefault();
            ...
            $(document).on('click', 'article .profileUserAvatar', function(event) {
                ...
                if (tutor === false && chatUserId === selectedUserId) {
            ...
            $('.list_contextmenu').on('click', '.contact', function() {
                ...
                if (window.parent.usersBlocked.indexOf(parseInt(contactId)) !== -1) {
